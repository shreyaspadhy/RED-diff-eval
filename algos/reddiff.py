...
class REDDIFF(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        ...
        for ti, si in tqdm(zip(reversed(ts), reversed(ss))):
            ...
            print(f"Step {ti} took {end_time - start_time} seconds")
            # #save for visualization
            if self.cfg.exp.save_evolution:
                if (
                    ti / ((self.cfg.exp.start_step - self.cfg.exp.end_step) // len(ts))
                ) % (len(ts) // 10) == 0:
                    mu_s = torch.cat((mu_s, mu.detach().cpu()), dim=3)
                    mu_fft_abs_s = torch.cat(
                        (
                            mu_fft_abs_s,
                            torch.fft.fftshift(
                                torch.abs(torch.fft.fft2(mu.detach().cpu()))
                            ),
                        ),
                        dim=3,
                    )
                    mu_fft_ang_s = torch.cat(
                        (
                            mu_fft_ang_s,
                            torch.fft.fftshift(
                                torch.angle(torch.fft.fft2(mu.detach().cpu()))
                            ),
                        ),
                        dim=3,
                    )
