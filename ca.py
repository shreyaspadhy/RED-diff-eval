...
def main(cfg: DictConfig):
    ...
    features = torch.tensor([top1, top5, count]).cuda()
    print(features)
    features_list = [
        torch.zeros_like(features).cuda() for i in range(dist.get_world_size())
    ]
    ...
    if dist.get_rank() == 0:
        ...
        results_file = os.path.join(cfg.results, "results.txt")
