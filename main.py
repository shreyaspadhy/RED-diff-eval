...
def main(cfg):
    ...
    cg_model = ClassifierGuidanceModel(
        model, classifier, diffusion, cfg
    )  # ?? what is the easiest way to call stable diffusion?
    ...
    print(
        "number of val batches : ", len(loader), "batch size : ", cfg.loader.batch_size
    )
    ...
    for it, (x, y, info) in tqdm(enumerate(loader)):
        ...
        # final_folder = f"{cfg.algo.lr}_{cfg.algo.grad_term_weight}"

        # samples_root = os.path.join(samples_root, final_folder)
        # if not os.path.exists(samples_root):
        # os.makedirs(samples_root)
        ...
        ssim = structural_similarity_index_measure(
            postprocess(x).cpu(), xo, reduction=None
        )
        print("ssim", ssim.mean())

        lpip = LearnedPerceptualImagePatchSimilarity().cuda()(
            postprocess(x).cuda(), torch.clamp(xo, -1.0, 1.0).cuda()
        )

        print("lpip", lpip.mean().item())

        with open(os.path.join(samples_root, "results.txt"), "a") as f:
            f.write(
                f"psnr: {psnr.mean().item()} ssim: {ssim.mean()} lpip: {lpip.mean().item()}\n"
            )
